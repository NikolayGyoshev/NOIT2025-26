...
import { z } from "zod";
import { insertRoomSchema, insertReservationSchema, insertReviewSchema, insertContactMessageSchema, type RoomExtra } from "@shared/schema";
  ...
  app.post(api.reservations.create.path, async (req, res) => {
    ...
    try {
      ...
      const reservationInputSchema = z.object({
        ...
        selectedExtras: z.array(z.string()).optional().default([]),
      });
      ...
      const selectedExtras: RoomExtra[] = (room.extras || [])
        .filter((extra) => input.selectedExtras.includes(extra.name));
      const extrasTotal = selectedExtras.reduce((sum, extra) => sum + extra.price, 0);
      const totalPrice = nights * room.price + extrasTotal;
...
async function seedDatabase() {
  ...
  if (existingRooms.length === 0) {
    await storage.createRoom({
      ...
      extras: [
        { name: "масаж", price: 5000 },
        { name: "храна и напитки", price: 3500 },
        { name: "наем на колело", price: 1200 },
      ],
      isAvailable: true
    });
    await storage.createRoom({
      ...
      extras: [
        { name: "масаж", price: 4500 },
        { name: "храна и напитки", price: 3000 },
        { name: "наем на колело", price: 1000 },
      ],
      isAvailable: true
    });
    await storage.createRoom({
      ...
      extras: [
        { name: "масаж", price: 6000 },
        { name: "храна и напитки", price: 4500 },
        { name: "наем на колело", price: 1500 },
      ],
      isAvailable: true
    });
  }
