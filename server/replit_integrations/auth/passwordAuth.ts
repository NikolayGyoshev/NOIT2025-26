...
export async function ensureDefaultUser() {
  const userEmail = process.env.DEFAULT_USER_EMAIL?.trim();
  const userPassword = process.env.DEFAULT_USER_PASSWORD;

  if (!userEmail || !userPassword) return;

  if (userPassword.length < 6) {
    console.warn("DEFAULT_USER_PASSWORD must be at least 6 characters. Default user not created.");
    return;
  }

  const existing = await authStorage.getUser(userEmail).catch(() => null);
  const passwordHash = await hashPassword(userPassword);

  await authStorage.upsertUser({
    id: (existing as any)?.id || `user-${Math.random().toString(36).slice(2, 10)}`,
    email: userEmail,
    passwordHash,
    firstName: process.env.DEFAULT_USER_FIRST_NAME?.trim() || existing?.firstName || "User",
    lastName: process.env.DEFAULT_USER_LAST_NAME?.trim() || existing?.lastName || "Account",
    profileImageUrl: existing?.profileImageUrl || null,
    isAdmin: false,
  });
}
