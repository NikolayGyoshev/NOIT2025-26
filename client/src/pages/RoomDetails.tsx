...
export default function RoomDetails() {
  ...
  const availableExtras = room.extras || [];
  const selectedExtrasWithPrice = availableExtras.filter((extra) => selectedExtras.includes(extra.name));
  const extrasTotal = selectedExtrasWithPrice.reduce((sum, extra) => sum + extra.price, 0);
            ...
            <div className="sticky top-24 bg-card border rounded-xl shadow-lg p-6 space-y-6">
              ...
              {availableExtras.length > 0 && (
                <div className="space-y-3 border rounded-lg p-4 bg-muted/10">
                  <p className="text-sm font-semibold">Екстри</p>
                  {availableExtras.map((extra) => (
                    <label
                      key={extra.name}
                      className="flex items-center justify-between text-sm gap-3 cursor-pointer rounded-md border border-border/40 bg-background/60 px-3 py-2 transition-colors hover:bg-secondary/30"
                    >
                      ...
                      <span className="font-medium">+ {(extra.price / 100).toFixed(2)} €</span>
                    </label>
                  ))}
                </div>
              )}
              ...
              <div className="space-y-4 pt-4 border-t">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{(roomTotal / 100).toFixed(2)} €</span>
                </div>
                {selectedExtrasWithPrice.length > 0 && (
                  <div className="flex justify-between text-sm">
                    <span>Екстри</span>
                    <span className="font-medium">{(extrasTotal / 100).toFixed(2)} €</span>
                  </div>
                )}
